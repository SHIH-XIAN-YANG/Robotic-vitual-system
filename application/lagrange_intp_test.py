...
class Intp():
    ...
    def __init__(self, iter:int=10):
        ...
        self.max_magnitude_deviation = 1
        self.min_magnitude_deviation = -0.15
        self.max_phase_shift = 0.15
        self.min_phase_shift = -0.15
    ...
    def run(self, tune_mode: ServoGain, k_min:float, k_max:float, feature_type: FeatureType=FeatureType.magnitude_deviation):
        ...
        if self.feature_type == FeatureType.magnitude_deviation:
            gain_init = self.gain[0] + self.magnitude_deviation[0]*(k_max - k_min) / (self.max_magnitude_deviation - self.min_magnitude_deviation)
        else:
            gain_init = self.gain[0] + self.phase_shift[0]*(k_max - k_min) / (self.max_phase_shift - self.min_phase_shift)
...
def main():
    ...
    interpolation.run(ServoGain.Velocity.value.kp, 1, 100, FeatureType.phase_shift)
    #interpolation.save_file()
    # interpolation.reset_RT605()
    ...
    interpolation.run(ServoGain.Position.value.kp, 1, 100, FeatureType.phase_shift)
    interpolation.save_file()
    # interpolation.reset_RT605()
